...
def bin2xyzw(bin_path, remove_plane=False):
    ...
    # read in LiDAR data  #CJ01
    # scan_data = np.fromfile(bin_path, dtype=np.float32).reshape((-1,4))
    # CJ01 use direct feeded
    scan_data = bin_path.reshape((-1,4))
    
    #scan_data = np.fromfile(bin_path, dtype=np.float32).reshape((-1,3))   #CJ01  

    # get x,y,z LiDAR points (x, y, z) --> (front, left, up)
    xyz = scan_data[:, 0:3]

    # keep only points in front of the sensor (positive x)
    xyz = xyz[xyz[:, 0] > 0]
